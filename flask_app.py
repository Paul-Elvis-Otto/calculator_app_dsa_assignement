...
def calculate():
    if request.method == "POST":
        ...
        try:
            ...
            return render_template(
                "calculator.html", printed_result="You cannot divide by zero"
            )
...
def circle_calculator():
    if request.method == "POST":
        try:
            ...
            if radius <= 0:
                return render_template(
                    "circle.html", error="Radius must be a positive number"
                )
            ...
            return render_template(
                "circle.html", perimeter=round(perimeter, 2), area=round(area, 2)
            )
        except ValueError:
            return render_template(
                "circle.html", error="Please enter a valid number for the radius"
            )
